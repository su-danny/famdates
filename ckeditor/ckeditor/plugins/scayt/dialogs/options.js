...
CKEDITOR.dialog.add('scaytcheck', function (a) {
    ...
    function l() {
        if (typeof document.forms['optionsbar_' + e] != 'undefined')return document.forms['optionsbar_' + e].options;
        return[];
    };
    function m() {
        if (typeof document.forms['languagesbar_' + e] != 'undefined')return document.forms['languagesbar_' + e].scayt_lang;
        return[];
    };
    function n(z, A) {
        if (!z)return;
        ...
        for (var C = 0; C < B; C++) {
            z[C].checked = false;
            if (z[C].value == A.toString())z[C].checked = true;
        }
    };
    var o = a.lang.scayt, p = [
        ...
        var A = 0;
        if (b)z.data.scayt.getCaption(a.langCode || 'en', function (B) {
            if (A++ > 0)return;
            c = B;
            s.apply(z);
            t.apply(z);
        ...
        var z = l();
        for (var A in z)z[A].checked = false;
        n(m(), '');
    }, contents: h}, r = CKEDITOR.plugins.scayt.getScayt(a);
    for (g = 0; g < f.length; g++) {
        if (f[g] == 1)h[h.length] = p[g];
    }
    if (f[2] == 1)i = 1;
    var s = function () {
        ...
        if (f[1] == 1) {
            for (F in A.rtl)D[D.length] = L(F, A.ltr);
            for (F in A.ltr)D[D.length] = L(F, A.ltr);
            D.sort(function (U, V) {
        ...
        if (f[0] == 1) {
            ...
            for (var B = 0, C = A.length; B < C; B++) {
                ...
                if (E) {
                    A[B].checked = false;
                    if (z.options[D.split('_')[0]] == 1)A[B].checked = true;
                    if (b)E.on('click', function () {
    ...
    function w(z) {
        z = String(z);
        var A = z.split(',');
        for (var B = 0, C = A.length; B < C; B += 1)d.getById(A[B]).$.style.display = 'inline';
    };
    function x(z) {
        z = String(z);
        var A = z.split(',');
        for (var B = 0, C = A.length; B < C; B += 1)d.getById(A[B]).$.style.display = 'none';
    };
    function y(z) {
