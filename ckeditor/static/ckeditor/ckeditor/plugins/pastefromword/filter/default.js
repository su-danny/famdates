...
(function () {
    ...
    a.firstChild = b.firstChild = function (u) {
        ...
        for (var w = 0; w < this.children.length; w++) {
            v = this.children[w];
            if (u(v))return v; else if (v.name) {
                v = v.firstChild(u);
                if (v)return v;
            }
        }
    ...
    b.addStyle = function (u, v, w) {
        var A = this;
        var x, y = '';
        if (typeof v == 'string')y += u + ':' + v + ';'; else {
            if (typeof u == 'object')for (var z in u) {
                if (u.hasOwnProperty(z))y += z + ':' + u[z] + ';';
            } else y += u;
            w = v;
        }
    ...
    function c(u) {
        ...
        x = u.attributes;
        if (B.exec(x.style))return;
        for (var D = 0; D < y; D++) {
            w = v[D];
            if (w.attributes.value && Number(w.attributes.value) == D + 1)delete w.attributes.value;
            z = B.exec(w.attributes.style);
            if (z)if (z[1] == A || !A)A = z[1]; else {
                A = null;
                break;
            }
        }
    ...
    function n(u) {
        u = u.toUpperCase();
        var v = l.length, w = 0;
        for (var x = 0; x < v; ++x)for (var y = l[x], z = y[1].length; u.substr(0, z) == y[1]; u = u.substr(z))w += y[0];
        return w;
    };
    function o(u) {
        u = u.toUpperCase();
    ...
    var p = 0, q = null, r, s = CKEDITOR.plugins.pastefromword = {utils: {createListBulletMarker: function (u, v) {
        ...
        return function (v, w, x) {
            u.setStyle(v, w);
            var y = {}, z = x.length;
            for (var A = 0; A < z; A++)y[x[A]] = u.getStyle(x[A]);
            return y;
        };
        ...
        for (var A = 0; A < y.length; A++) {
            ...
            if (z.name in CKEDITOR.dtd.$listItem) {
                ...
                u.children = [];
                for (var F = 0, G = y.length; F < G; F++)u.add(y[F]);
            }
        }
        delete u.name;
        w['cke:list'] = 1;
            ...
            if ('cke:li' == w.name) {
                ...
                if (!G) {
                    ...
                    if (F && k[F][E].test(G[1])) {
                        H = F;
                        I = E;
                    } else for (var L in k)for (var M in k[L]) {
                ...
                if (I && I != (H == 'ol' ? 'decimal' : 'disc'))x.addStyle('list-style-type', I);
                if (H == 'ol' && G) {
                    switch (I) {
                        case 'decimal':
                            J = Number(G[1]);
                ...
                B = x;
                A = z;
            } else if (C)C = A = B = null;
        }
        for (K = 0; K < D.length; K++)c(D[K]);
        C = A = B = r = q = p = null;
        ...
        return function (w, x) {
                ...
                for (var H = 0; H < u.length; H++) {
                    if (u[H]) {
                        ...
                        if (B.match(D) && (!E || C.match(E))) {
                            B = G || B;
                            v && (F = F || C);
                            if (typeof F == 'function')F = F(C, x, B);
                            if (F && F.push)B = F[0], F = F[1];
                            if (typeof F == 'string')y.push([B, F]);
                            return;
                        }
                    }
                }
            ...
            for (var z = 0; z < y.length; z++)y[z] = y[z].join(':');
        ...
        if (v.name.indexOf('cke:') == -1)return false;
        ...
        return{elementNames: [
            ...
            var P;
            if (CKEDITOR.env.gecko && (P = y.applyStyleFilter))P(O);
        }, $: function (O) {
            var P = O.name || '', Q = O.attributes;
            if (P in w && Q.style)Q.style = A([
                [/^(:?width|height)$/, null, J]
            ])(Q.style) || '';
            if (P.match(/h\d/)) {
                O.filterChildren();
            ...
            if (CKEDITOR.env.gecko) {
                ...
                if (Q) {
                    Q.replace(/[\n\r]/g, '').replace(/(.+?)\{(.+?)\}/g, function (S, T, U) {
                        T = T.split(',');
                        var V = T.length, W;
                        for (var X = 0; X < V; X++)CKEDITOR.tools.trim(T[X]).replace(/^(\w+)(\.[\w-]+)?$/g, function (Y, Z, aa) {
                            Z = Z || '*';
                            aa = aa.substring(1, aa.length);
                            if (aa.match(/MsoNormal/))return;
                            if (!R[Z])R[Z] = {};
                            if (aa)R[Z][aa] = U; else R[Z] = U;
                        });
                    });
            ...
            } else B(x['format_' + (x.enterMode == CKEDITOR.ENTER_P ? 'p' : 'div')])(O);
        }, div: function (O) {
            var P = O.onlyChild();
            if (P && P.name == 'table') {
                var Q = O.attributes;
            ...
            if (O.getAncestor('thead'))O.name = 'th';
            ...
            var P = O.attributes;
            if (P && !P.href && P.name)delete O.name; else if (CKEDITOR.env.webkit && P.href && P.href.match(/file:\/\/\/[\S]+#/i))P.href = P.href.replace(/file:\/\/\/[^#]+/i, '');
        }, 'cke:listbullet': function (O) {
            if (O.getAncestor(/h\d/) && !x.pasteFromWordNumberedHeadingToList)delete O.name;
        }}, attributeNames: [
            [/^onmouse(:?out|over)/, ''],
            [/^onload$/, ''],
            [/(?:v|o):\w+/, ''],
            ...
            [/^margin$|^margin-(?!bottom|top)/, null, function (O, P, Q) {
                if (P.name in {p: 1, div: 1}) {
                    ...
                    if (Q == 'margin')O = K(Q, O, [R])[R]; else if (Q != R)return null;
                    if (O && !e.test(O))return[R, O];
                }
                return null;
            }],
            [/^clear$/],
            [/^border.*|margin.*|vertical-align|float$/, null, function (O, P) {
                if (P.name == 'img')return O;
            }],
            [/^width|height$/, null, function (O, P) {
                if (P.name in {table: 1, td: 1, th: 1, img: 1})return O;
            }]
        ] : [
            [/^mso-/],
            [/-color$/, null, function (O) {
                if (O == 'transparent')return false;
            ...
            if (P.name in v.$tableContent)return false;
        }, border: function (O, P) {
            if (P.name in v.$tableContent)return false;
    ...
    CKEDITOR.cleanWord = function (u, v) {
        if (CKEDITOR.env.gecko)u = u.replace(/(<!--\[if[^<]*?\])-->([\S\s]*?)<!--(\[endif\]-->)/gi, '$1$2$3');
