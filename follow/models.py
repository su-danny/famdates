...
class FollowManager(models.Manager):
    ...
    def create(self, user, obj, **kwargs):
        ...
        follow.target = obj
        follow.is_blocked = kwargs.get('is_blocked') == True
        follow.save()
        return follow
    ...
    def get_or_create(self, user, obj, **kwargs):
        ...
        if kwargs.get('is_blocked'):
            if self.is_blocked(user, obj):
                objs = self._get_follows(obj).filter(user=user)[0]
                return obj, False
            else:
            ...
            if not self.is_following(user, obj):
                return self.create(user, obj, **kwargs), True
            return self.get_follows(obj).filter(user=user)[0], False

    def is_following(self, user, obj, **kwargs):
        """ Returns `True` or `False` """
        if isinstance(user, AnonymousUser):
            return False
        return 0 < self.get_follows(obj).filter(user=user, is_blocked=False).count()

    def is_blocked(self, user, obj):
        if isinstance(user, AnonymousUser):
            return False
        try:
            follows = self._get_follows(obj).filter(user=user)
            for f in follows:
                print (f, f.is_blocked)
                if f.is_blocked:
                    return True
        except Exception, e:
            print e
    ...
    def _get_follows(self, model_or_obj_or_qs):
    ...
    def get_follows(self, model_or_obj_or_qs):
        ...
        if isinstance(model_or_obj_or_qs, QuerySet):
            return self.filter(**{'%s__in' % fname: model_or_obj_or_qs}).exclude(is_blocked=True)

        if inspect.isclass(model_or_obj_or_qs):
            return self.exclude(**{fname: None}).exclude(is_blocked=True)

        return self.filter(**{fname: model_or_obj_or_qs}).exclude(is_blocked=True)
