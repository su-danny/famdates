...
@register.filter
def is_blocked(user, obj):
    """
    Returns `True` in case `user` is following `obj`, else `False`
    """
    return Follow.objects.is_blocked(user, obj)
...
def get_following(parser, token):
    ...
    """
    bits = token.split_contents()
    return GetFollowingNode(*bits[1:])
...
class GetFollowingNode(template.Node):
    def __init__(self, obj, tpl=None):
        self.obj = template.Variable(obj)

    def render(self, context):
        print '????'
        current_profile = self.obj.resolve(context)
        followers = [follow.user.get_profile() for follow in Follow.objects.get_follows(current_profile)]
        followers.sort(cmp)
        following = [follow.target_userprofile for follow in
                     Follow.objects.filter(user=current_profile.user, is_blocked=False)]
        following.sort(cmp)
