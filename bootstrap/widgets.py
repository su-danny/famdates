from django.forms.widgets import Input, RadioInput, RadioFieldRenderer, RadioSelect, TextInput, TimeInput
...
class PrependedText(TextInput):
    def render(self, name, value, attrs=None):
        ...
        return '<span class="add-on">%s</span>%s' % (
        prepend_text, super(PrependedText, self).render(name, value, attrs))


class AppendPrependText(TextInput):
    def render(self, name, value, attrs=None):
        append_text, prepend_text = self.attrs.get('append_text', ''), self.attrs.get('prepend_text', '')
        return '<span class="add-on">%s</span>%s<span class="add-on">%s</span>' % (
