...
class Migration(SchemaMigration):
    def forwards(self, orm):
        # Adding field 'UserProfile.deactivated'
        db.add_column('account_userprofile', 'deactivated',
                      self.gf('django.db.models.fields.BooleanField')(default=False),
                      keep_default=False)

        # Adding M2M table for field blocked_users on 'UserProfile'
        db.create_table('account_userprofile_blocked_users', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('userprofile', models.ForeignKey(orm['account.userprofile'], null=False)),
            ('user', models.ForeignKey(orm['auth.user'], null=False))
        ))
        db.create_unique('account_userprofile_blocked_users', ['userprofile_id', 'user_id'])


    def backwards(self, orm):
        # Deleting field 'UserProfile.deactivated'
        db.delete_column('account_userprofile', 'deactivated')

        # Removing M2M table for field blocked_users on 'UserProfile'
        db.delete_table('account_userprofile_blocked_users')
