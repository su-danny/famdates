...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Deleting field 'Child.interest'
        db.delete_column('account_child', 'interest_id')

        # Adding field 'Child.parent'
        db.add_column('account_child', 'parent',
                      self.gf('django.db.models.fields.related.ForeignKey')(default=1, related_name='parent', to=orm['account.UserProfile']),
                      keep_default=False)

        # Removing M2M table for field parent on 'Child'
        db.delete_table(db.shorten_name('account_child_parent'))

        # Adding M2M table for field interest on 'Child'
        m2m_table_name = db.shorten_name('account_child_interest')
        db.create_table(m2m_table_name, (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('child', models.ForeignKey(orm['account.child'], null=False)),
            ('interestcategory', models.ForeignKey(orm['account.interestcategory'], null=False))
        ))
        db.create_unique(m2m_table_name, ['child_id', 'interestcategory_id'])


    def backwards(self, orm):
        # Adding field 'Child.interest'
        db.add_column('account_child', 'interest',
                      self.gf('django.db.models.fields.related.ForeignKey')(to=orm['account.InterestCategory'], null=True, blank=True),
                      keep_default=False)

        # Deleting field 'Child.parent'
        db.delete_column('account_child', 'parent_id')

        # Adding M2M table for field parent on 'Child'
        m2m_table_name = db.shorten_name('account_child_parent')
        db.create_table(m2m_table_name, (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('child', models.ForeignKey(orm['account.child'], null=False)),
            ('userprofile', models.ForeignKey(orm['account.userprofile'], null=False))
        ))
        db.create_unique(m2m_table_name, ['child_id', 'userprofile_id'])

        # Removing M2M table for field interest on 'Child'
        db.delete_table(db.shorten_name('account_child_interest'))
    ...
    models = {
        'account.child': {
            ...
            'interest': ('django.db.models.fields.related.ManyToManyField', [], {'symmetrical': 'False', 'to': "orm['account.InterestCategory']", 'null': 'True', 'blank': 'True'}),
