...
from messages.utils import format_subject, format_quote
...
class IntegrationTestCase(TestCase):
    ...
    def testOutboxEmpty(self):
        ...
        self.assertEquals(response.status_code, 200)
        self.assertEquals(response.templates[0].name, 'messages/outbox.html')
        self.assertEquals(len(response.context['message_list']), 0)
    ...
    def testTrashEmpty(self):
        ...
        self.assertEquals(response.status_code, 200)
        self.assertEquals(response.templates[0].name, 'messages/trash.html')
        self.assertEquals(len(response.context['message_list']), 0)
