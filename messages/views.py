...
def inbox(request, template_name='messages/inbox.html'):
    ...
    unread_number = Message.objects.unread_for(request.user)
    user = request.user.get_profile()
    event_list = user.events.all()
    print request.GET.get('recipient')
    return render_to_response(template_name, {
        'message_list': message_list,
        'compose_form': ComposeForm(initial={'recipient':request.GET.get('recipient', '').split(',')}),
...
def compose(request, recipient=None, form_class=ComposeForm,
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if request.is_ajax():
                return HttpResponse(json.dumps({'success': True, 'message': "success"}))

            return HttpResponseRedirect(success_url)
        else:
            if request.is_ajax():
                return HttpResponse(json.dumps({'success': False, 'message': str(form.errors)}))

    else:
        form = form_class()
        if recipient is not None:
            recipients = [u for u in User.objects.filter(pk=recipient)]
